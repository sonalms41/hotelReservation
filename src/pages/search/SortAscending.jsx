...
const SortAscending = (props) => {
	...
	useEffect(() => {
		...
		const sortAsc = arrData.sort(function (a, b) {
		...
		if (sortAsc.length >= 1) {
			for (let i = 0; i < nListsToShow; i++) {
				if (sortAsc[i] !== undefined) {
					currentShowLists.push(sortAsc[i]);
				}
			}
		}
		setAscendingOrder(currentShowLists);
	...
	return (
		<>
			...
			<div className="prop-lists">
				...
				{ascendingOrder.length >= 1 &&
					ascendingOrder.map((property, i) => {
						return (
								...
								<div className="prop-item__detail utl-col-2">
									<div className="utl-row utl-row-1 flex-aC-jSB">
										<div className="utl-col-1">
											<h3 className="hotel-title" onClick={handleRouteLink}>
												<span
													onClick={() =>
			...
			<div className="property-control">
				<div className="property-control__sum flex-aC-jFE">
					<strong>
						{allPropertiesLength}{" "}
						{ascendingOrder.length > 1 ? "Properties" : "Property"} found
						{/*{location}*/}
					</strong>
				</div>
				<ul className="property-control__show flex-aC-jFE">
					<li>
						<span>
							{" "}
							{ascendingOrder.length}{" "}
							{ascendingOrder.length > 1 ? "Properties" : "Property"} shown
						</span>{" "}
						|
					</li>
					<li>
